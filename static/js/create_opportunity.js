...
(function(){
  ...
  function saveFormLocally(){
    ...
    const data={};
    getRequiredFields().forEach(f=>{ const el=document.getElementById(f); if(el) data[f]=el.value; });
    localStorage.setItem(KEY, JSON.stringify(data));
  }
  function restoreFormLocally(){
    if(editing) return;
    try{ const raw=localStorage.getItem(KEY); if(!raw) return; const data=JSON.parse(raw);
      getRequiredFields().forEach(f=>{ const el=document.getElementById(f); if(el && data[f]!==undefined) el.value=data[f]; });
    }catch(e){ console.warn('Restore draft failed', e); }
  }
  function clearFormLocally(){ if(!editing) localStorage.removeItem(KEY); }

  function highlightRequiredFields(missing){ missing.forEach(f=>{ const el=document.getElementById(f); if(el) el.classList.add('required-highlight'); }); }
  ...
  function publishOpportunity(){
    ...
    const missing=[]; getRequiredFields().forEach(f=>{ const el=document.getElementById(f); if(el && !el.value) missing.push(f); });
  ...
  function previewOpportunity(){
    const data={}; getRequiredFields().forEach(f=>{ const el=document.getElementById(f); data[f]=el?el.value:''; });
  ...
  // Expose
  window.CO = { getRequiredFields, saveFormLocally, restoreFormLocally, clearFormLocally, publishOpportunity, saveAsDraft, confirmCancel, previewOpportunity, showPreviewModal, closePreviewModal, highlightRequiredFields, clearRequiredHighlights };

  document.addEventListener('DOMContentLoaded', ()=>{
    restoreFormLocally();
    // Auto-save typing
    getRequiredFields().forEach(f=>{ const el=document.getElementById(f); if(el){ el.addEventListener('input', ()=>{ saveFormLocally(); el.classList.remove('required-highlight'); }); }});
