...
(function(){
  ...
  savedProfilesRaw.forEach(sp => { const p = sp.profiles || sp; const id = (p && (p.id || sp.profile_id)); if(id!=null) window.savedProfilesMap[String(id)] = p; });
  window.savedOpportunitiesMap = {};
  savedOpportunitiesRaw.forEach(so => { const o = so.opportunities || so; const id = (o && (o.id || so.opportunity_id)); if(id!=null) window.savedOpportunitiesMap[String(id)] = o; });

  function qs(id){ return document.getElementById(id); }

  // Modal controls
  window.openModal = function(id){ const m = qs(id); if(!m) return; m.classList.remove('hidden'); document.body.style.overflow='hidden'; };
  window.closeModal = function(id){ const m = qs(id); if(!m) return; m.classList.add('hidden'); document.body.style.overflow='auto'; };
  ...
  document.addEventListener('keydown', e => { if(e.key==='Escape'){ document.querySelectorAll('.fixed.inset-0:not(.hidden)').forEach(m=>m.classList.add('hidden')); document.body.style.overflow='auto'; } });
  ...
  window.toast = function(message, isError){
    ...
    t.className = `fixed bottom-4 right-4 px-4 py-2 rounded shadow text-sm ${isError?'bg-red-600 text-white':'bg-green-600 text-white'}`;
    t.style.opacity='1';
  ...
  window.updateSavedCounts = function(){
    ...
    if(pCountEl){
      ...
      if(btn){ if(cnt <= 2) btn.classList.add('hidden'); else btn.classList.remove('hidden'); }
      if(cnt === 0){ const list = qs('saved-profiles-list'); if(list){ const p=document.createElement('p'); p.className='text-gray-500 text-center py-8'; p.textContent='No saved profiles yet'; list.replaceWith(p);} }
    }
    ...
    if(oCountEl){
      ...
      if(btn){ if(cnt <= 2) btn.classList.add('hidden'); else btn.classList.remove('hidden'); }
      if(cnt === 0){ const list = qs('saved-opportunities-list'); if(list){ const p=document.createElement('p'); p.className='text-gray-500 text-center py-8'; p.textContent='No saved opportunities yet'; list.replaceWith(p);} }
    }
  };
  ...
  window.deleteOpportunity = async function(id, elementId){
    ...
    try {
      ...
      if(data.success){ const el = qs(elementId); if(el) el.remove(); toast('Opportunity deleted'); }
      else toast(data.error || 'Delete failed', true);
    } catch(e){ toast('Error deleting opportunity', true); }
  };
  ...
  window.unsaveOpportunity = async function(id, elementId){
    ...
    try {
      ...
      const data = await res.json();
      if(data.success){
        const el = qs(elementId); if(el) el.remove();
        const otherIds = [ `saved-opp-${key}`, `modal-saved-opp-${key}` ];
        otherIds.forEach(oid=>{ if(oid!==elementId){ const o = qs(oid); if(o) o.remove(); }});
      ...
      } else toast(data.error || 'Remove failed', true);
    } catch(e){ toast('Error removing saved opportunity', true); }
  };
  ...
  window.unsaveProfile = async function(id, elementId){
    ...
    } catch(e){ toast('Error removing profile', true); }
  };
  ...
  function replenish(type){
    ...
    for(const rawId of order){
      ...
      const raw = (type==='profile'? savedProfilesRaw : savedOpportunitiesRaw).find(r=>{
        const obj = r.profiles || r.opportunities || r; const rid = (obj.id || r.profile_id || r.opportunity_id); return String(rid) === id; });
      if(!raw) continue;
