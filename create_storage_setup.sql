...
VALUES ('profile-pictures',
        ...
        ARRAY['image/jpeg', 'image/png', 'image/webp', 'image/gif']::text[]) ON CONFLICT (id) DO NOTHING;
...
CREATE
POLICY "Allow public viewing of profile pictures"
ON storage.objects 
FOR
SELECT
...
CREATE
POLICY "Allow users to update own profile pictures"
ON storage.objects 
FOR
UPDATE
    TO authenticated
...
CREATE
POLICY "Allow users to delete own profile pictures"
ON storage.objects 
FOR DELETE
TO authenticated 
USING (
